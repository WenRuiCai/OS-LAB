...
page_init(void)
{
     ...
     * filed to 1) */
        int used_mem = (freemem & 0x1fffffff) >> 12;
        struct Page *go = pages;
        int i=0;
        for (;i<npage;i++,go++) {
                if (i<used_mem) {
                        go->pp_ref = 1;
                        ...
                        go->pp_ref = 0;
                        LIST_INSERT_HEAD(&page_free_list, go, pp_link);
                }
        }
...
{
        struct Page *ppage_temp = LIST_FIRST(&page_free_list);
